...
float APCompressor::applyFBCompression(float sample)
{
  ...
  if (y_dB > (threshold_ + kneeWidth_ / 2))
    ...
    gain_sc = y_dB + ((1 / ratio_ - 1) * powf((y_dB - threshold_ + kneeWidth_ / 2), 2)) /
                         (2 * kneeWidth_);  // Compression with ramp
  else
    gain_sc = y_dB;
...
float APCompressor::applyRMSCompression(float sample)
{
  ...
  {
    // attack mode
    gainSmooth = -sqrt(((1 - alphaA) * powf(gainChange_dB, 2)) + (alphaA * powf(prevGainSmooth_, 2)));
  }
  else
  {
    // release mode
    gainSmooth = -sqrt(((1 - alphaR) * powf(gainChange_dB, 2)) + (alphaR * powf(prevGainSmooth_, 2)));
  }
